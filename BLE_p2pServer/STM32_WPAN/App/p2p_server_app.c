...
/* Private macros ------------------------------------------------------------*/
/* USER CODE BEGIN PM */
#define LOWORD(l)   ((uint8_t)((l) & 0x00FF))
#define HIWORD(l)   ((uint8_t)(((l) & 0xFF00) >> 8))
/* USER CODE END PM */
...
static void Send_ADC_Value_Notification(void);
/* USER CODE END PFP */
...
void P2P_SERVER_APP_Init(void)
{
  ...
  UTIL_SEQ_RegTask( 1U << CFG_TASK_SEND_NOTIF_ID, UTIL_SEQ_RFU, Send_ADC_Value_Notification);
...
/* USER CODE BEGIN FD */
void Send_ADC_Value_Notification(void)
{
  ...
  /* Update notification data (ADC result) */
  a_P2P_SERVER_UpdateCharData[0] = HIWORD(adc_result[0]); //HI
  a_P2P_SERVER_UpdateCharData[1] = LOWORD(adc_result[0]); //LO
  a_P2P_SERVER_UpdateCharData[2] = HIWORD(adc_result[1]);
  a_P2P_SERVER_UpdateCharData[3] = LOWORD(adc_result[1]);
  a_P2P_SERVER_UpdateCharData[4] = HIWORD(adc_result[2]);
  a_P2P_SERVER_UpdateCharData[5] = LOWORD(adc_result[2]);

  /* Update notification data length */
  p2p_server_notification_data.Length = (p2p_server_notification_data.Length) + 6;

  if(P2P_SERVER_APP_Context.Switch_c_Notification_Status == Switch_c_NOTIFICATION_ON)   //Skip to check the flag of notification.
  {
    LOG_INFO_APP("-- P2P APPLICATION SERVER : Send_ADC_Value_Notification\n");
    LOG_INFO_APP("-- P2P APPLICATION SERVER : ADC Result[0] = 0x%02X%02X\n", a_P2P_SERVER_UpdateCharData[0], a_P2P_SERVER_UpdateCharData[1]);
    LOG_INFO_APP("-- P2P APPLICATION SERVER : ADC Result[1] = 0x%02X%02X\n", a_P2P_SERVER_UpdateCharData[2], a_P2P_SERVER_UpdateCharData[3]);
    LOG_INFO_APP("-- P2P APPLICATION SERVER : ADC Result[2] = 0x%02X%02X\n", a_P2P_SERVER_UpdateCharData[4], a_P2P_SERVER_UpdateCharData[5]);

    P2P_SERVER_UpdateValue(P2P_SERVER_SWITCH_C, &p2p_server_notification_data);
  }
  else
  {
    LOG_INFO_APP("-- P2P APPLICATION SERVER : Send_ADC_Value_Notification failed! - NOTIFICATION DISABLED\n");
  }
