...
__USED static void gatt_Notification(GATT_CLIENT_APP_Notification_evt_t *p_Notif)
{
  ...
  switch (p_Notif->Client_Evt_Opcode)
  {
    ...
    case P2P_NOTIFICATION_INFO_RECEIVED_EVT:
    {
      ...
      LOG_INFO_APP("-- P2P APPLICATION CLIENT : ADC Result[0] = 0x%02X%02X\n", p_Notif->DataTransfered.p_Payload[0], p_Notif->DataTransfered.p_Payload[1]);
      LOG_INFO_APP("-- P2P APPLICATION CLIENT : ADC Result[1] = 0x%02X%02X\n", p_Notif->DataTransfered.p_Payload[2], p_Notif->DataTransfered.p_Payload[3]);
      LOG_INFO_APP("-- P2P APPLICATION CLIENT : ADC Result[2] = 0x%02X%02X\n", p_Notif->DataTransfered.p_Payload[4], p_Notif->DataTransfered.p_Payload[5]);
#else
